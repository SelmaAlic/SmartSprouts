...
class Circle:
    ...
    def start(self,pos):
        self.dragging=True
        self.ox = self.x-pos[0]; self.oy=self.y-pos[1]
    ...
    def update(self,pos):
        if self.dragging:
            self.x = pos[0]+self.ox; self.y = pos[1]+self.oy

class Bin:
    def __init__(self,lo,hi,idx,total,area):
        self.lo,self.hi=lo,hi
        w = area.width//total
        self.rect=pygame.Rect(area.left+idx*w, area.bottom-BIN_HEIGHT, w, BIN_HEIGHT)
...
def run_game():
    ...
    circle=Circle(LEVEL_RANGES[0][0][0],
                  LEVEL_RANGES[0][-1][1],content)
    ...
    while running:
        clock.tick(FPS)
        remaining = max(0, TIMER_SECONDS - int(time.time()-start))
                        ...
                        if b.contains(circle):
                            if b.lo<=circle.number<=b.hi:
                                ...
                                if level<len(LEVEL_THRESHOLDS) and score>=LEVEL_THRESHOLDS[level]:
                                    ...
                                    txt = msg_font.render(f"🎉 Level {level+1}! 🎉",True,NAVY)
                                    overlay.blit(txt,((w-txt.get_width())//2,(h-txt.get_height())//2))
    ...
    screen.blit(final,((WIDTH-final.get_width())//2,HEIGHT//2+20))
    pygame.display.flip()
