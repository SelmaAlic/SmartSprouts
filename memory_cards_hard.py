...
def get_level_images(level, used_images):
    ...
    return [pygame.image.load(os.path.join(image_folder, f"memory_hard{i}.png")).convert_alpha() for i in selected]
...
def memory_cards_hard(username):
    ...
    def check_achievements():
        if sum(m is not None for m in level_mistakes) >= 5:
            unlock_sticker_achievement("finish_5_levels")
        if all(m is not None for m in level_mistakes):
            unlock_sticker_achievement("finish_all_levels")
        if sum((m is not None and m <= 3) for m in level_mistakes) >= 5:
            unlock_sticker_achievement("finish_5_levels_3_mistakes")
        if all((m is not None and m <= 3) for m in level_mistakes):
            unlock_sticker_achievement("finish_all_levels_3_mistakes")

    def initialize_level():
        nonlocal cards, card_states, flipped_indices, wait_time, level_completed, mistakes
        nonlocal rows, cols, start_x, start_y, card_w, card_h, grid_width, grid_height
        images = get_level_images(level, used_images)
        cards = images * 2
        mistakes = 0
    ...
    while running:
        ...
        if mistakes >= 5 and not show_try_again:
            show_try_again = True
            try_again_start_time = pygame.time.get_ticks()

        if show_try_again and pygame.time.get_ticks() - try_again_start_time > 1500:
            mistakes = 0
            show_try_again = False
            rows, cols, start_x, start_y, card_w, card_h, grid_width, grid_height = initialize_level()
        ...
        mistake_text = font.render(f"Mistakes: {mistakes}/5", True, (200, 50, 50))
        screen.blit(mistake_text, (current_width - 220, 40))
        ...
        if not level_completed and all(state == "matched" for state in card_states):
            ...
            level_mistakes[level] = mistakes
            check_achievements()
            upsert_progress(
                username=username,
                game_name=GAME_NAME,
                best_level=max(level + 1, progress['best_level']),
                best_time=None,  
                mistakes=mistakes
            )
        ...
        if show_try_again:
            ...
            try_text = font.render("Try Again!", True, (255, 255, 255))
